...
```{r echo=FALSE, warning=FALSE, fig.cap='Monthly Ozone (2018)'}
epa_data %>% 
  filter(year(Date) == 2018, !is.na(Ozone)) %>% 
  ggplot(aes(x = month(Date, label = TRUE, abbr = TRUE), y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone: 2018", x = NULL, y = "Ozone (µg/m^3)")
```

```{r echo=FALSE, warning=FALSE, fig.cap='Monthly Ozone (2019)'}
epa_data %>% 
  filter(year(Date) == 2019, !is.na(Ozone)) %>% 
  ggplot(aes(x = month(Date, label = TRUE, abbr = TRUE), y = Ozone)) +
  geom_boxplot() +
  labs(title = "Monthly Ozone: 2019", x = NULL, y = "Ozone (µg/m^3)")
```
